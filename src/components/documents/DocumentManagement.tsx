...
export const DocumentManagement: React.FC = () => {
  ...
  const handleUploadFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE_BYTES) {
      ...
      alert(
        `⚠️ PENOLAKAN UPLOAD DOKUMEN:\n\n` +
          `File "${file.name}" ditolak oleh sistem karena ukurannya ${fileSizeMB} MB, melebihi batas maksimum 1 MB.\n\n` +
          `Peringatan: Silakan kompres atau kecilkan ukuran file Anda di bawah 1 MB, lalu lakukan unggah ulang.`
      );
      e.target.value = '';
      return;
    }
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-5">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <label className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 text-xs font-semibold cursor-pointer transition-colors shrink-0">
              ...
              {/* Informational Banner for Max File Size */}
              <div className="flex items-center gap-2 rounded-lg bg-amber-50/80 border border-amber-200 p-2.5 text-[11px] text-amber-900">
                <FileText className="h-4 w-4 text-amber-600 shrink-0" />
                <span>
                  <strong>Batas Ukuran File:</strong> Maksimal <strong>1 MB</strong> per file (PDF, DOC, DOCX). Jika ukuran melebihi 1 MB, file akan ditolak dan Anda perlu mengunggah ulang file yang lebih kecil.
                </span>
              </div>
