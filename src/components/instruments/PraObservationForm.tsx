...
import { Sparkles, Save, Check, X, FileText, Upload, CheckCircle2, ShieldAlert } from 'lucide-react';
  ...
  return (
    <form onSubmit={handleFormSubmit} className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
        <div className="border-b border-slate-100 pb-2 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          <div>
          ...
          {!isPrincipal && (
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-[11px] font-semibold">
              <ShieldAlert className="h-3.5 w-3.5 text-amber-600 shrink-0" />
              Khusus Kepala Sekolah (Guru Read-Only)
            </div>
          )}
        </div>

        {!isPrincipal && (
          <div className="rounded-lg bg-amber-50/80 border border-amber-200 p-3 text-amber-900 text-xs flex items-center gap-2">
            <ShieldAlert className="h-4 w-4 text-amber-600 shrink-0" />
            <span>
              Bagian 1 (Centang & Catatan Telaah Modul Ajar/RPP) hanya dapat diisi dan diubah oleh <strong>Kepala Sekolah</strong>.
            </span>
          </div>
        )}
                  ...
                  <tr key={item.id} className="hover:bg-slate-50">
                    ...
                    <td className="py-3 px-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button
                          ...
                          title={isPrincipal ? "Sesuai (Ya)" : "Hanya dapat diubah Kepala Sekolah"}
                        >
                        ...
                        <button
                          ...
                          title={isPrincipal ? "Belum Sesuai (Tidak)" : "Hanya dapat diubah Kepala Sekolah"}
                        >
                          <X className="h-4 w-4" />
                        </button>
                      </div>
                    ...
                    <td className="py-3 px-3">
                      <input
                        ...
                        placeholder={isPrincipal ? "Catatan temuan..." : "Hanya diisi oleh Kepala Sekolah"}
                        className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:bg-white focus:outline-none focus:ring-1 focus:ring-indigo-600 disabled:bg-slate-100 disabled:text-slate-500 disabled:cursor-not-allowed"
                      />
                    </td>
                  </tr>
                );
