...
import { Sparkles, Save, CheckCircle2, ShieldAlert } from 'lucide-react';
...
export const ObservationForm: React.FC<Props> = ({ data, teacher, onSave }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {!isPrincipal && (
        <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 text-amber-900 text-xs flex items-center gap-3 shadow-sm">
          <ShieldAlert className="h-5 w-5 text-amber-600 shrink-0" />
          <div>
            <div className="font-bold text-sm text-amber-950">Mode Lihat (Read-Only) — Observasi Kelas</div>
            <div>
              Pengisian dan penilaian Lembar Observasi Kelas hanya dapat dilakukan oleh <strong>Kepala Sekolah / Penilai</strong>. Guru hanya dapat melihat hasil observasi ini.
            </div>
          </div>
        </div>
      )}
                    ...
                    <tr key={item.id} className="hover:bg-slate-50">
                        ...
                        <div className="flex items-center justify-center gap-1">
                          {[1, 2, 3, 4].map((s) => (
                            <button
                              ...
                              title={isPrincipal ? `Pilih Skor ${s}` : 'Hanya diisi Kepala Sekolah'}
                            >
                              {s}
                            </button>
                          ))}
                      ...
                      <td className="py-3 px-3">
                        <textarea
                          ...
                          placeholder={isPrincipal ? "Fakta / bukti teramati di kelas..." : "Belum ada bukti teramati dari pengamat."}
                          className="w-full p-2 bg-slate-50 border border-slate-200 rounded text-xs text-slate-800 focus:bg-white focus:outline-none focus:ring-1 focus:ring-indigo-600 disabled:bg-slate-100 disabled:text-slate-500 disabled:cursor-not-allowed"
                        />
                      </td>
                    </tr>
                  );
        ...
        <div className="space-y-4 text-xs">
          <div>
            ...
            <textarea
              ...
              placeholder={isPrincipal ? "Tuliskan catatan supervisor..." : "Hanya diisi oleh Kepala Sekolah."}
              className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-600 disabled:bg-slate-100 disabled:text-slate-500 disabled:cursor-not-allowed"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={isPrincipal ? "Tuliskan rekomendasi perbaikan..." : "Hanya diisi oleh Kepala Sekolah."}
              className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-600 disabled:bg-slate-100 disabled:text-slate-500 disabled:cursor-not-allowed"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={isPrincipal ? "Tuliskan aksi tindak lanjut..." : "Hanya diisi oleh Kepala Sekolah."}
              className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-600 disabled:bg-slate-100 disabled:text-slate-500 disabled:cursor-not-allowed"
            />
          </div>
        </div>
